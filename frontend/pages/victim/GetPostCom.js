...
export default function VicPro() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/victim/searchComplain/${inputId}`
      );
    //   const response = await axios.get(
    ...
    //   );
      ...
      if (response.data) {
        ...
        router.push(`/victim/GetComplain/${inputId}`);
      } else {
  ...
  return (
      <>
     <Layout />
     <h1 class="text-2xl font-bold text-black-500 mb-4" align="center"> View Post Complain</h1>
    <form onSubmit={handleSubmit}>
      <label
      for="ID"
      class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
>
        Enter your Post Complain ID:
        <input
          type="text"
